...
def insertDBAptTradeDev(result):
    dbConfig = {
        'host': 'localhost',
        'user': 'root',
        'password': 'gd16741',
    ...
    try:
        ...
        cursor = conn.cursor()
        sql = """
        INSERT INTO apt_trade_dev (APTDONG, APTNM, APTSEQ, BONBUN, BUBUN, BUILDYEAR, BUYERGBN, CDEALDAY, CDEALTYPE, DEALAMOUNT, DEALDAY, DEALMONTH, DEALYEAR, DEALINGGBN, EXCLUUSEAR, FLOOR, JIBUN, LANDCD, LANDLEASEHOLDGBN, RGSTDATE, ROADNM, ROADNMBONBUN, ROADNMBUBUN, ROADNMCD, ROADNMSEQ, ROADNMSGGCD, SGGCD, SLERGBN, UMDCD, UMDNM)
        ...
        # json > tuple 변환
        dataTuples = [(item['aptDong'], item['aptNm'], item['aptSeq'], item['bonbun'], item['bubun'],item['buildYear'],item['buyerGbn'], item['cdealDay'], item['cdealType'], item['dealAmount'], item['dealDay'], item['dealMonth'], item['dealYear'], item['dealingGbn'], item['excluUseAr'], item['floor'], item['jibun'], item['landCd'], item['landLeaseholdGbn'], item['rgstDate'], item['roadNm'], item['roadNmBonbun'],item['roadNmBubun'],item['roadNmCd'],item['roadNmSeq'], item['roadNmSggCd'], item['sggCd'], item['slerGbn'], item['umdCd'], item['umdNm']) for item in result]
